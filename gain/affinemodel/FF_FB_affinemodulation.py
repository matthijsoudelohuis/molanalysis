...
my_savefig(fig,savedir,'FF_FB_poprate_Corr_GR_L23_%dsessions' % nSessions)
# my_savefig(fig,savedir,'FF_FB_poprate_Corr_GR_L5_%dsessions' % nSessions)
...
ax.set_xlim([0,np.nanmax(np.nanmean(mean_resp_split,axis=(0,1,3)))*1.1])
ax.set_ylim([0,np.nanmax(np.nanmean(mean_resp_split,axis=(0,1,3)))*1.1])
plt.tight_layout()
sns.despine(fig=fig, top=True, right=True,offset=3)
# my_savefig(fig,savedir,'FF_FB_affinemodulation_%dGRsessions' % (nSessions), formats = ['png'])
...
for iparam in range(2):
    ...
    if iparam==1:
        sns.barplot(data=np.abs(data_gainregress[:,:,iparam]),palette=clrs_arealabelpairs,ax=ax)
    else: 
        sns.barplot(data=data_gainregress[:,:,iparam],palette=clrs_arealabelpairs,ax=ax)
                    # ax=ax)
...
plt.tight_layout()
sns.despine(fig=fig, top=True, right=True,offset=3)
# my_savefig(fig,savedir,'FF_FB_affinemodulation_popcoupling_%dGRsessions_L5' % (nSessions), formats = ['png'])
my_savefig(fig,savedir,'FF_FB_affinemodulation_popcoupling_%dGRsessions_L23' % (nSessions), formats = ['png'])
