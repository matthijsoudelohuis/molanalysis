...
#         trial_types = ['X', 'Y']
#         tn = self.select_trials(only_correct=False, trial_type=trial_types)
...
# # selected_channel_inds = []
# #
# # ind = np.where(session.lfp_data['Area'] == area)[0]
# # selected_channel_inds.append(ind)
# #
# # if layer is not None:
# #     ind = np.where(session.lfp_data['layer'] == layer)[0]
# #     selected_channel_inds.append(ind)
# #
# # q=0.1
# # min_depth = np.quantile(session.lfp_data['ChannelY'], q)
# # max_depth = np.quantile(session.lfp_data['ChannelY'], 1 - q)
# # ind = np.where(np.logical_and(session.lfp_data['ChannelY'] >= min_depth,
# #                session.lfp_data['ChannelY'] <= max_depth))
# #
# # selected_inds = list(set.intersection(*map(set, selected_channel_inds)))
# #
# # selected_ind = np.random.choice(selected_inds)
# # # ChannelY micrometers
# #
# # n_channels

# # never select 53-54 (my 52-53)
...
#     session.load_data(load_lfp=True)
#     # LFP PARAMETERS

#     # TODO put this stuff in get_equispaced_channel_ids

#     # select area
#     #area_ind = np.where(session.lfp_data['area'] == area)[0]

#     # get the indices of channels of the area sorted by depth
#     #sorted_area_ind = area_ind[np.argsort(session.lfp_data['ChannelY'][area_ind])]

#     #assert set(sorted_area_ind) == set(area_ind)

#     # # get the indices of the channels
#     # sorted_channel_ids = session.channel_id[sorted_area_ind]
#     #
#     # # build indices at equispace location (generic function)
#     # equispaced_indices = session.get_equispaced_channel_indices(sorted_channel_ids.shape[0],
#     #                                                              n_channels=n_channels,
#     #                                                              exclude_first_and_last=True)
#     #
#     # # get equispaced channel ids
#     # equispaced_channel_ids = sorted_channel_ids[equispaced_indices]



#     # selected_ch_inds = sorted_area_ind[equispaced_indices]
#     # selected_ch_depths = sorted_depth[equispaced_indices]
#     # selected_ch_Y = session.lfp_data['ChannelY'][selected_ch_inds]
#     # selected_ch_X = session.lfp_data['ChannelX'][selected_ch_inds]
#     #
#     # selected_ch_XY = np.vstack((selected_ch_X, selected_ch_Y)).T
#     #
#     # cell_id = '20122018081447072'
#     # # TODO this does not work now that there are duplicates
#     # cell_ind = np.argwhere(session.cell_id == cell_id)[0]
#     # cell_channel_id = session.spike_data['ch'][cell_ind]
#     #cell_channel_xy
...
#         freq_bands = session.make_frequency_bands(low, mid, high, spacing_low,
#                                                   spacing_high)
...
#         trial_times = session.get_aligned_times(trial_numbers, time_before_in_s=time_before_in_s,
#                                                 time_after_in_s=time_after_in_s)
...
#         phase_by_trial = np.vstack([bp[2, :] for bp in binned_phase])
